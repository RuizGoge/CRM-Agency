...
- **TTI: MEASURED and enforced (2026-08-03).** P20 = 2300 ms, from a median of 2251 ms by Lighthouse's `interactive` audit against the production build and the `perf-500` fixture, under Lighthouse's own mobile preset. Enforced by `tests/e2e/tti.spec.ts` in the `lh-ci` profile — **not** by the nightly Actions tier the row used to promise, which was abandoned rather than built: §9.4.1 makes the absence of a payment method this project's cost control, so a scheduled job is a gate that gets switched off. **Gate 11 is closed.** ⚠️ Unlike P12/P13 this number is **machine-dependent**; a slower runner measures more, and `monotonic_down` will refuse to loosen it.

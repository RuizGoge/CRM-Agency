...
describe('break-glass releases exactly two verdicts and never a third', () => {
  ...
  it('expires on read at sixty minutes, with no job involved', async () => {
    // The override is live at its start; asked about an instant 61 minutes
    // later, it is not. Nothing ran in between — expires_at is GENERATED ALWAYS
    // from started_at and evaluated by the query.
    //
    // 🔴 STARTED_AT IS PINNED, and it was not. This test read
    // `started_at + 61 minutes` off a row whose start is `now()`, so the instant
    // it asked about was a wall-clock time — and the verdict it expects is
    // `blocked_calling_window`, which only holds when that instant falls
    // OUTSIDE Central's legal hours. It passed at night and failed by day: found
    // at 09:17 Central, where 61 minutes later is 10:18 and `allow` is the
    // CORRECT answer. Every other assertion in this file already uses a fixed
    // instant for exactly this reason.
    //
    // Anchoring the start so that "61 minutes later" IS `FL_MORNING` keeps the
    // intent whole — the override expires on read, with no job — and borrows the
    // one constant this file already trusts to close the Central window.
    await sql`
      UPDATE app.break_glass_override
         SET started_at = ${FL_MORNING}::timestamptz - interval '61 minutes'

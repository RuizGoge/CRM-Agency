...
### 🧬 CONSTRUÍ EL BELT-AND-BRACES Y ME SALTEÉ EL PRIMARIO — 0076 LO CORRIGE (2026-08-14, noche)
Migración **0076** · `ref.protected_surface()` · `security.protected_object` + `_history` · `security.assert_protected_objects()` (PO001) · `assertProtectedSurfaceUnchanged` (BOOT017/018) · `scripts/deploy-credential.sql`. **833 → 846 tests · 80 archivos.** `verify` verde. **Verificado en pantalla: la app arranca.**

🔴 **INVERTÍ LA PRIORIDAD DEL CORPUS Y NO ME DI CUENTA HASTA RELEER `05c`.** §11.11.3 se titula literalmente *"Platform reality, and it is why this cannot be the primary"* — los event triggers exigen superuser y el corpus registra **en cuatro lugares** que el diseño no debe depender de ese grant. §11.11.4 se titula *"the digest chain, **which needs no superuser and is the primary**"*. Lo que construí a la tarde fue §11.11.3. **El primario estaba entero sin construir**, medido: no existían `security.seal`, `harden_run`, `protected_object`, `catalog_digest` ni `ref.sealed_signature`.

🎯 **Y NO SON REDUNDANTES — hay un caso que lo prueba, y es el más filoso disponible.** `ALTER TABLE app.earnings_ledger DISABLE TRIGGER t_immutable_earnings_ledger` **apaga el append-only del registro de plata**. El guard de event triggers **no lo detiene**: el tag es un `ALTER TABLE` pelado, RLS queda intacto y la identidad reportada es la TABLA, no el trigger. PO001 lo nombra: `CHANGED trigger:app.earnings_ledger.t_immutable_earnings_ledger`. Al revés también: PO001 corre después de que la migración commiteó, y el guard la habría revertido en la misma transacción.

🎯 **LA SUPERFICIE SE COMPUTA POR CLASE, NO SE TIPEA.** `05c` §2563 lista ~30 objetos por nombre y **cinco de esos nombres no existen en este árbol** (medido). Transcribirla habría dado un gate rojo en su primera corrida y borrado en la segunda. Cuatro clases: cada política RLS, cada trigger `t_immutable_*`, cada cuerpo SECURITY DEFINER y los flags de RLS. Hoy: **201 objetos — 90 políticas, 55 definers, 44 RLS, 12 triggers.**

⚠️ **LAS PARTICIONES QUEDAN AFUERA Y ESO ES LO QUE SALVA AL GATE.** `event_outbox` gana una partición por día y pierde una cada catorce, cada una con políticas y triggers que generó `harden()`. Incluidas, la superficie cambiaría **todas las noches**, PO001 dispararía por calendario y alguien lo apagaría en una semana — la forma exacta en que un gate se vuelve un comentario.

🔴 **Y ENCONTRÉ QUE EL DEPLOY ESTABA ROTO DESDE AYER, POR MI PROPIO CAMBIO DE CREDENCIAL.** `drizzle-kit migrate` como `crm_migrator` falla con `permission denied for schema drizzle`: el esquema donde drizzle lleva su registro es de `crm`. **Ninguna migración podía aplicarse.** Tres cosas lo taparon a la vez: drizzle-kit **sale 1 e imprime NADA**; `npm run verify` **no corre el deploy**; y el grado decía `(b)` todo el tiempo — el aislamiento era real, sólo que además impedía desplegar, y el grado no tiene opinión sobre eso. Una base nueva tampoco lo habría mostrado: `crm_migrator` tiene CREATE sobre la base y se crearía su propio esquema. Arreglado fuera de banda en `scripts/deploy-credential.sql`, dentro de `npm run db:guard`.

⚠️ **EL LÍMITE, DICHO Y NO ESCONDIDO: EL ATAQUE CIRCULAR SIGUE ABIERTO.** El rol del deploy **es dueño de `security.protected_object`**, así que una migración que debilite algo *y* reescriba su propia línea base produce un digest auto-consistente y pasa. §11.11.4 lo nombra y lo cierra con la cadena de sellos de §7.6.2 — `ci/seal-manifest.jsonl`, `security.seal`, boot comparando cabezas de manifiesto — y **nada de eso existe**. Lo que sí rige hoy: PO001 caza todo debilitamiento que no reescriba su propia base en el mismo deploy, y **donde el guard está instalado es mucho más fuerte**, porque tocar una política se niega sin importar qué diga la base.

📌 **Dos tests ajenos se pusieron rojos y los dos tenían patrones demasiado amplios.** `one-decay-threshold` buscaba `conname ILIKE '%rot%'` y matcheó "p**rot**ected"; angostado a `%rotting%`, que sigue cazando `tenant_rotting_before_cold`. `snapshot-chain` exigía que todo `CREATE INDEX` estuviera declarado en Drizzle **sin filtrar esquema**, mientras su propio comentario dice que `security` deliberadamente no se modela; ahora excluye `security`/`authz` por nombre en vez de permitir sólo `app|ref`, para no tirar los tres índices reales de `auth`.

📌 **Y la firma de siempre otra vez, ahora en mis propios tests:** tres aserciones pasaban solas y fallaban en la suite. No estaban mal — `crm_test` es compartido y varios archivos **cambian el esquema a propósito** (`silo.test.ts` crea tablas para probar que `harden()` las rechaza), así que la superficie ya había derivado. El archivo ahora **nivela la línea base contra el mundo que encuentra** y mide sólo lo que él mismo cambia.
...
🔴 **LA 0075 REINTRODUJO EL DEFECTO QUE VENÍA A ELIMINAR.** `security.property_b_grade()` toma el rol del deploy como **parámetro con default `'crm'`**. La función conoce privilegios de roles; **no puede saber a qué rol apunta `MIGRATION_DATABASE_URL`**. Ese default era una afirmación no verificada sobre configuración de deploy — justo la especie que todo el ejercicio existe para eliminar, reintroducida por el objeto construido para eliminarla. Se vio al cambiar la credencial: el grado del rol que ahora despliega pasó a `(b)` y **`property-b.test.ts` siguió VERDE**, porque fijaba el grado de `crm`, que sigue siendo superuser y simplemente ya no despliega. **El pin era cierto y había dejado de ser sobre algo.**

**El arreglo es dejar de preguntar y empezar a observar.** El script corre como parte del deploy, sobre `MIGRATION_DATABASE_URL`, y califica `current_user` — el rol del deploy **por construcción**, no por aserción. No hay parámetro que equivocar.
...
⚠️ **N13 QUEDA ABIERTO, MEDIDO Y NO TOCADO.** El presupuesto de búsqueda (120 ms) falla **de manera intermitente** dentro de la suite completa (123,1 ms · 137,3 ms) y pasa cuando el archivo corre solo — con el mismo código, y también **sin** los cambios de esta sesión, así que no es una regresión de acá. Medido el porqué: **no son los datos.** Proceso Node limpio contra la MISMA base sucia que deja la suite (36 tenants, 50.088 filas) da **p95 100,7 ms · mediana 43,5 ms**. Lo que se mueve es el proceso, no el servidor — 77 archivos de tests de presión de heap. El presupuesto se llama `perf.N13_search_server_p95` y **mide el proceso, no el servidor**: la misma lección que L2-P midiendo el reloj del poller. **No se tocó el número** — cambiar la metodología de un gate de performance es decisión de Jorge, no un arreglo al cierre de sesión.
...
| rol | grado |
|---|---|
| `crm` (~~corre las migraciones~~ — **ya no despliega, 2026-08-14**) | **`(a)+(c)`** — es superuser |
...
✅ **EL SEGUNDO HUECO, CERRADO EL MISMO DÍA — y el arreglo cambió lo que la falla dice.** El discado a T+5 s devolvía `blocked_calling_window`: el reloj decidía antes que la supresión, y ninguna elección de estado lo arregla porque entre las 06:00 y las 12:00 UTC Estados Unidos entero está durmiendo. Ahora va bajo break-glass, igual que `dial-gate.test.ts`, abierto **sólo alrededor de esa llamada** y cerrado en un `finally` (verificado: cero overrides vivos al terminar). **El veredicto pasó de `blocked_calling_window` a `allow`**, que es la señal correcta y mucho más filosa: la puerta **permite** discar a un número que texteó STOP. Antes la ventana de llamada tapaba el defecto; ahora la falla dice la cosa verdadera.

⚠️ **Y el override NO debilita lo que se afirma**, que es por qué es el instrumento correcto: libera exactamente `blocked_timezone_unknown` y `blocked_calling_window`, mientras supresión y guarda de grabación siguen atando — `dial-gate.test.ts` tiene una aserción parada que dice que **un admin bajo break-glass sigue sin poder discar un STOP**.
...
🔴 **EL AGUJERO ERA EL LEGAL.** `message.received` estaba en los 49 canónicos desde la 0042 **sin emisor**, así que ningún texto entrante producía evento, así que `consent.stop_recorder` no tenía qué consumir y **ningún STOP que un lead mandara alcanzaba `suppression_list`**. §581 nombra el fallo como *"un STOP honrado en SMS pero no en el discador"* — y era peor: no se honraba en ninguno de los dos, porque nadie lo anotaba.

🎯 **LA ASERCIÓN QUE IMPORTA ES DE PUNTA A PUNTA:** un lead textea STOP y **el discado siguiente a ese número lo rechaza la puerta con `blocked_suppressed`**. Antes de la 0069 ese discado pasaba.

- **La cadena vive DENTRO de `message_merge`, no en un consumidor.** §581: *"una lista de supresión eventualmente consistente es un sistema eventualmente legal."* La fila de consentimiento, la de supresión y la del mensaje commitean juntas; no hay ventana donde el mensaje exista y la supresión no.
- 🔴 **MATCH DE MENSAJE COMPLETO, JAMÁS SUBSTRING, y ése es todo el argumento de corrección.** `ILIKE '%stop%'` suprimiría con *"please don't stop calling me"*. La supresión es **de todo el tenant**, la puerta la lee en cada intento, y este slice **no trae camino de des-supresión** — así que un falso positivo borra un lead del alcance de todos los vendedores para siempre. §1797 llama a un STOP forjado *"una denegación de servicio contra el libro de un vendedor"*; un substring es la misma denegación, autoinfligida y sin atacante. Las cuatro frases peligrosas están en el test y leen `reply`.
- **Cuelga del dueño resuelto, que es §1797(3) sosteniéndose estructuralmente.** Los dos lookups de mapeo ya exigen `verified_at IS NOT NULL`, así que un webhook forjado a un número no verificado resuelve `unmapped` y **nunca llega a la cadena**.
- **`xmax = 0` es lo que impide que una re-declaración sea un segundo STOP.** G2 midió que Aloware **re-declara el mismo mensaje con bytes distintos** segundos después, así que la función corre más de una vez por mensaje real por diseño. Sin esa guarda, cada re-declaración apendaba otra fila de consentimiento y otra de supresión.
- **Supresión sin canal (`p_channel => NULL`)** — §2721: un STOP suprime llamada **y** texto. Acotarlo a SMS dejaba al discador libre de llamar a quien acababa de decir basta.
- ⚠️ **NO existe intención de re-suscripción.** `START`/`UNSTOP`/`YES` son keywords reales y `suppression_kind` hasta tiene `start`, pero `intent_hint` en el catálogo canónico es `stop|help|reply|out_of_office` y no tiene miembro para eso. Inventarlo sería un bug, no una feature, así que un START lee `reply` y **re-suscribirse sigue siendo algo que sólo hace una persona.**
- ⚠️ **Y una frase en lenguaje natural NO suprime.** *"I want to stop"* lee `reply`, que es la conducta estándar de los carriers, pero es un residual honesto: el vendedor ve el mensaje en su timeline y decide.

🔴 **UN DEFECTO QUE ENCONTRÓ EL TEST Y NINGUNA REVISIÓN HABRÍA VISTO.** Las migraciones corren como `crm` (superuser), así que `app.sms_intent_of` nace con dueño `crm`; `message_merge` es definer y **desde la 0062 corre como `crm_migrator`**, al que el `REVOKE ALL … FROM PUBLIC` deja sin EXECUTE. La primera corrida murió con `permission denied for function sms_intent_of` en **todo** mensaje entrante. Lo arregla `SELECT security.own_to_migrator()`. **Es invisible hasta que un definer llama a la función nueva** — las tres de la 0068 se alcanzan desde sesiones `crm_app` directas y por eso no lo notaron.

⚠️ **ESTO NO CIERRA G6/P24, y decirlo importa.** La puerta agrega un **reloj**: la fila de supresión dentro de **5 s** detrás de un backlog de 20.000 mensajes. Eso necesita `ref.job_registry.priority` con las líneas `compliance | interactive | bulk` de **`05c` §11.7**, que **no existe en este árbol**. Hoy un STOP detrás de una tormenta llega tarde, y llegar tarde es un fallo legal y no sólo una puerta roja. (⚠️ `05-architecture` §484 dice que ese eje *"debe un fallo antes del Sprint 0"* — **ese texto está vencido**: `05c` §11.7 ES el fallo.)

### 🔌 EL BORDE DE INGESTA POR FIN TIENE UNA CREDENCIAL QUE SE LE PUEDA DAR (2026-08-12)
Migración **0068** · `app/routes/api/webhook-endpoints.ts` · sección nueva en `/admin/integration-health` · `webhook-endpoint-writer.test.ts`. **715 → 731 tests · E2E 120 → 122** (el a11y de la pantalla de admin, en los dos perfiles). `verify` verde.

🔴 **EL HALLAZGO, Y ERA UN CAMINO DE PÉRDIDA PERMANENTE VIVO.** `app.webhook_endpoint` es `definer_only` y **no tenía escritor en ningún lado** — ni en `app/`, ni en la semilla, ni en una migración. La Puerta 6 lo encontró tropezándose: el arnés de la tormenta intentó sembrar su fila como `crm_app`, se comió un permission denied, y tuvo que insertar **como dueño** para poder correr. La consecuencia en producción es que no hay fila de endpoint, `app.webhook_ingest` no resuelve tenant para ningún token, y **el borde contesta 401 a toda entrega real** — con G2 midiendo que Aloware **no reintenta nunca**. Cada llamada y cada SMS descartados en la puerta, y **todos los contadores de `/admin/integration-health` leyendo cero mientras pasa**, porque nunca se guardó nada que contar.

🎯 **LA ASERCIÓN QUE IMPORTA ES EL IDA Y VUELTA, y a propósito no es una comparación de strings.** El emisor y `webhook_ingest` tienen que hashear idéntico —`pg_catalog.sha256(pg_catalog.convert_to(token,'UTF8'))`— y cualquier otra grafía produce una fila **estructuralmente válida, que pasa todos los CHECK, y que no resuelve para nada**: un token que la operadora pega bien y contesta 401 para siempre, sin error en ninguna parte. Por eso el test emite un token y se lo da al `webhook_ingest` **real**.

- **La lista NO vive en su propia ruta**, y el gate del registro es la razón: un descriptor declara UN método y `route-registry` afirma que coincida con el montaje, así que un archivo con `loader` **y** `action` llevaría un descriptor que describe honestamente la mitad de sí mismo. La lectura se fue a `/api/integration-health` — misma pantalla, mismo scope, un fetch en vez de dos.
- **El admin se chequea adentro del definer, no en la ruta.** `app.scope_is_admin()` relee `app_user.role` para el par (tenant, usuario) **sellado por la 0067**; no es un reclamo del llamador. Sin esa línea, cualquier sesión de vendedora que alcance la función acuña una credencial que resuelve toda su agencia a quien la tenga.
- **pgcrypto no está instalado** —medido en la 0067, no supuesto—, así que el token son cuatro `gen_random_uuid()` sin guiones: 128 caracteres hex, ~488 bits del mismo CSPRNG.
- ⚠️ **El revoke lleva paso de confirmación y NO el undo de 5 s de la casa, y la excepción está argumentada en el lugar.** Desde que el revoke commitea el borde contesta 401 y Aloware no reintenta: todo lo enviado durante una ventana de undo **se pierde de verdad** en vez de reproducirse. Dos clics le ganan a cinco segundos de pérdida silenciosa.
- **El estado vacío es la razón de ser de la sección.** Verificado en pantalla con la admin: sin endpoint vivo la tarjeta roja dice *"Nothing is connected"* **mientras el bloque de abajo dice "Nothing needs attention"** — ese contraste es exactamente el defecto, y ahora se lee.
- **La pantalla de admin nunca había estado bajo el gate de axe-core.** Existía desde el trabajo de dead-letter y ningún caso la visitaba; una superficie fuera del scan no está gateada, diga lo que diga la constitución. Entró ahora porque le puse un **formulario**.

⚠️ **LO QUE ESTO NO CIERRA:** no hace legal una entrega (la puerta de compliance está en otro lado y no cambió), no cierra **G6/P24** —sigue sin haber sniff de STOP en el ingreso, así que `message.received` es inalcanzable y la aserción protegida sigue sin sujeto—, y **nada acá verifica que la operadora haya pegado el token en el panel de Aloware**: la primera entrega real es la única prueba.

### 🔐 EL SILO DEJA DE SER UNA CONVENCIÓN Y PASA A SER UNA REGLA (2026-08-12)
Migración **0067** · `tests/integration/identity-seal.test.ts`. **709 → 715 tests · E2E 120/120.** `verify` verde.

🔴 **EL HALLAZGO, MEDIDO ANTES DE CERRARLO.** Dentro de la sesión de una vendedora real, como `crm_app`:

```
SELECT count(*) FROM app.contact;                       -->  42
SELECT set_config('app.user_id', <una colega>, true);
SELECT count(*) FROM app.contact;                       -->   3
```

Toda política RLS del esquema, el predicado de `timeline_read` y el SM404 de `stage_move` descansaban sobre `app.current_user_id()`, que leía un GUC que el rol de aplicación podía **sobrescribir**. El silo era una convención que la aplicación observaba, no una regla que la base imponía. **Después de la 0067 la misma falsificación devuelve CERO filas**, no el libro de la colega — la dirección fail-closed, porque un reclamo sin sello resuelve a NULL y `owner_user_id = NULL` es falso para toda fila.

⚠️ **LO QUE SE INTENTÓ PRIMERO Y NO HACE NADA, anotado para que nadie lo repita.** PG15 agregó `GRANT SET ON PARAMETER`, y `REVOKE SET ON PARAMETER app.user_id FROM PUBLIC` **se acepta sin error y no cambia nada**: `app.user_id` es un GUC placeholder, así que no se guarda ninguna fila de ACL —`pg_parameter_acl` quedó vacío— y `set_config` siguió funcionando en una conexión `crm_app` real. Un revoke que reporta éxito y no hace nada es peor que no tenerlo.

**La forma que sí funciona es la que `app.scope_is_global()` ya usaba: no confiar en el reclamo, verificarlo.** `begin_request` acuña un sello sobre (tenant, usuario) con un secreto que vive en `security` —un esquema donde `crm_app` no tiene ni USAGE, así que la tabla no es sólo ilegible: es **innombrable**— y `current_user_id` rechaza cualquier reclamo sin sello coincidente.

🎯 **Y EL PRESUPUESTO CAZÓ LA PRIMERA VERSIÓN.** Anidar un SECURITY DEFINER dentro de otro hacía que cada sentencia pagara dos llamadas no-inlineables más una lectura de heap: N13 midió **p95 = 121,7 ms contra un presupuesto de 120**. Fusionarlas en una sola llamada lo bajó a **83,9 ms**. Un presupuesto no se afloja para que pase un build, y ese 1,4% de exceso era todo el aviso.

**Dos puertas existentes lo cazaron también, las dos con razón:** `definer-tenancy` (las dos funciones nuevas **son** de lo que está hecho el contexto de tenant, así que quedan exentas por la primera razón en su forma más pura) y `silo.test.ts`, cuyo helper ponía los tres GUCs a mano y llamaba a eso *"one unit of work, exactly as the application performs it"* — que era **exactamente la falsificación que esta migración cierra**. Ahora abre la puerta de producción, y el test que reclamaba un scope que su rol no tenía **lo falsifica como `crm_app` dentro de una sesión real**. Más fuerte que antes, y el comentario es cierto por primera vez.

⚠️ **LO QUE SIGUE ABIERTO:** `app.tenant_id` solo sigue siendo un reclamo pelado. Falsificar los dos juntos no produce sello válido, así que toda política owner-scoped cierra — pero las clases `tenant_scoped_read` scopean sólo por `current_tenant()` y lo seguirían. Sellar el tenant necesita un valor acuñado antes de que exista usuario, y `begin_request` toma los dos juntos.

### 🧑‍💼 EL DEMO TIENE ADMIN, Y UN TEST DEJA DE PUDRIRSE (2026-08-12)
**Dos cosas chicas que bloqueaban cosas reales.** `/admin/integration-health` **no tenía espectador posible**: todos los usuarios sembrados eran `seller`, y `dead_letter` y `admin_alert` son `tenant_admin_only`. La superficie existía y estaba testeada desde el trabajo de alertas y nadie podía abrirla. Verificado en pantalla con la admin nueva.

El admin se siembra **antes** de `refuseToSeedTwice`, y la ubicación es el argumento: esa guarda existe por el **ledger**, que es append-only sin job de recómputo. Una fila de usuario es idempotente, así que sembrarla antes de la negativa deja que una base ya sembrada consiga un admin **sin el reset completo** que el ledger obliga. ⚠️ Se llama Valeria y no Dana porque `app_user_email_uidx` rechazó el primer intento — `emailLocalPart` toma el nombre de pila y Dana Reyes ya tenía `dana@demo.test`. La constraint haciendo su trabajo sobre la semilla misma.

Y `card-anatomy` afirmaba el literal `Going cold · 9d` contra una tarjeta que la semilla retrofecha **al momento de sembrar**. Se puso roja a medianoche **dos veces en esta sesión**, las dos por tener razón: el lead realmente tenía diez días. Ahora el número se **deriva** y la aserción es la invariante que §2.7 dicta de verdad — las dos representaciones existen y **coinciden**. Eso no se pudre.

### 🧱 LAS TRES PUERTAS QUE QUEDABAN, CERRADAS (2026-08-12)
Migraciones **0062 / 0063 / 0064** · cuatro aserciones de arranque nuevas (BOOT007–BOOT015) · `definer-owner.test.ts` y `timeline-forgery.test.ts`. **693 → 697 tests · 66 → 68 archivos.** `verify` verde. Diseñadas por tres pistas paralelas, cada una con verificador adversarial contra la base viva; **la primera fue declarada BROKEN y reescrita con seis correcciones.**

**0062 — EL DUEÑO DE LOS DEFINERS DEJA DE SER SUPERUSER.** `crm_migrator` era NOLOGIN, no era dueño de nada y no tenía USAGE sobre `app`; las migraciones corrían como `crm`, superuser. O sea que **los ~41 cuerpos SECURITY DEFINER corrían como un rol que no coincidía ni con `p_app` ni con `p_sys`** sobre tablas con FORCE RLS — funcionaban sólo por bypass, y **las 66 políticas `p_sys` no se habían ejecutado nunca**. Invisible donde el dueño ES superuser; fatal donde no: `stage_move` levanta *"new row violates row level security policy for table event_log"* en el **primer cierre**, después de un deploy verde, adentro del camino de la plata. 189 objetos movidos.
- 🎯 **La mutación que importa:** borrar el `PERFORM security.own_to_migrator()` y **la migración se niega a commitear** — OWN003, 185 objetos ajenos, 65 relaciones RLS ajenas. No un test rojo: un deploy que no entra.
- 🎯 **Y la que prueba que las políticas pasaron a ser portantes:** `DROP POLICY p_sys ON app.event_log` y cerrar una venta → RLS la rechaza. Antes de la 0062 ese DROP no cambiaba absolutamente nada.
- **El hook va en `security.managed_relations()`, NO en `harden()`.** Copiar las 185 líneas de `harden()` era la forma obvia y es la que hay que rechazar: la 0048 registra ese cuerpo quedando viejo **dos veces** por una rama que lo copió temprano y aterrizó tarde. `managed_relations()` tiene ocho líneas y nunca fue copiada.
- ⚠️ **`pgboss` necesitó grant en dos lugares y ninguno sobra:** la 0062 cubre el caso donde el esquema ya existe, `jobs-schema.ts` el de base nueva donde `db:jobs` corre **después** de las migraciones. Sin eso todo webhook de Aloware levanta excepción — que es exactamente como se manifestó, **15 tests rojos en tres archivos**.

**0063 — EL TABLERO PÚBLICO DE PLATA NO TIENE ESCRITOR DE ROL DE APLICACIÓN.** `app.ledger_append` estaba otorgada a `crm_app` y su cuerpo **valida el tenant y nada más**. Reproducido como sesión de vendedora común: una llamada apendó **$999.999.999,99 a nombre de OTRA vendedora**, citando un evento que no existe y un negocio que no existe. Un INSERT directo en la misma sesión fue rechazado: la puerta de la tabla aguantaba, la de la función estaba abierta.
- 🔴 **Tres hechos, tres trabajos, y ninguno es respaldo del otro:** el revoke cierra la alcanzabilidad desde una ruta; un CHECK en `earnings_ledger` acota **una** entrada y ata a un definer futuro —que no necesita grant— y a la consola SQL del proveedor; y un CHECK en `leaderboard_projection`, **sin el cual la prosa de esta migración era falsa**: medido con los otros dos aplicados, un UPDATE movió el tablero a $999.999.999.999,99 con el conteo de filas del ledger sin cambiar. **El número que miran cincuenta personas era más barato y más silencioso de falsificar que el registro detrás.**
- La cota del tablero es **derivada, no elegida**: `abs(total) <= entry_count * 1e9`. Aguanta contra las 27 filas vivas.
- ⚠️ **El censo lo tuve mal la primera vez:** el borrador nombraba cuatro funciones con grant a PUBLIC y decía que eran todas; **son veinte**. Por eso el strip es un loop sobre `pg_proc`, restringido a funciones que `crm_app` ya alcanza por nombre — lo que lo vuelve demostrablemente gratis.
- ⚠️ **El titular es POR ENTRADA, no en agregado.** `crm_app` tiene INSERT sobre `contact` y `opportunity`, así que INSERT → INSERT → `stage_move(…, 10000000, 'annual')` en loop suma $100.000 por vuelta con el loop como único límite. Nada acá acota el total de una vendedora.

**0064 — EL TIMELINE DEJA DE ACEPTAR RESPUESTAS Y PASA A ACEPTAR SUJETOS.** `app.timeline_upsert` tomaba contacto, dueño, **actor**, instante, kind, ref, payload y proveniencia como parámetros del llamador, **sin predicado de propiedad en ninguna parte**. Una ruta escribía sobre la Actividad de una colega con `actor_user_id` puesto en esa colega — y `timeline_read` renderiza eso como `timeline.actor.you`, así que **la falsificación aparecía en la historia de la víctima como acción propia**. De las cinco migraciones de endurecimiento es **la única con síntoma clase (a)**.
- `app.timeline_project(uuid)`: un argumento, todo lo demás leído del evento adentro del definer. No queda nada que falsificar, por eso no necesita predicado — y **no podía tenerlo**: el único llamador es el relay bajo `withSystemWork`, donde `current_user_id()` es NULL, así que un predicado habría rechazado el 100% de las proyecciones de producción **pasando todos los tests del repositorio**.
- 🔴 **Un chequeo de scope no estaba disponible, y eso se midió:** `crm_app` pone `app.scope_mode` en `'system'` desde adentro de su propio request, por dos caminos distintos. Una compuerta sobre ese string es una compuerta que el llamador setea.
- **El mapa se mudó a `ref.timeline_projection` con FK a `event_consumer`, y la FK cazó una al entrar:** `consent.updated` estaba en el mapa TypeScript y `contacts` **nunca** se suscribió, así que el kind `consent` era inalcanzable desde la 0059 y nadie lo decía.
- 🎯 **Y el proyector tenía un bug de lógica ternaria que escribí yo y cazó la suite:** `jsonb_typeof(payload -> 'contact_id') <> 'string'` **nunca dispara** cuando la clave está ausente — `jsonb_typeof(NULL)` es NULL, `NULL <> 'string'` es NULL, y plpgsql lee una condición NULL como falsa. La fila caía al INSERT y moría en el NOT NULL. Entrega trabada, encontrada por `outbox-relay.test.ts` a un minuto de aplicar la migración.
- ⚠️ **Cierra la FABRICACIÓN, no el CAUSAR una proyección.** `app.outbox_claim` está otorgada a `crm_app` y **no tiene predicado de tenant** — está en la lista exenta de `definer-tenancy.test.ts` por eso mismo —, así que una transacción con scope de vendedora obtiene todo id de evento sin entregar de todos los tenants. Lo que consigue es que un evento **verdadero** se proyecte, que es lo que el relay iba a hacer igual; el merge orden-independiente es lo que hace que dé lo mismo.

🔴 **LO MÁS GRANDE QUE ENCONTRÓ ESTA SESIÓN NO ES NINGUNA DE LAS TRES, y no lo cierra ninguna.** `crm_app` puede reescribir su propia identidad a mitad de transacción: `set_config('app.user_id', <colega>, true)` y **todo el silo lo sigue** — verificado, el conteo pasó de 42 contactos a 3. Cada política RLS del esquema, el predicado de `timeline_read` y el SM404 de `stage_move` descansan sobre que la aplicación no haga eso. Es el piso sobre el que están paradas las otras cinco migraciones.

⚠️ **Y sigue sin haber propiedad (b) en ningún lado:** las cuatro aserciones nuevas viven en el árbol que el mismo actor edita. Borrar una línea de `pool.ts` las desarma y nada se pone rojo.
> **Corregido el 2026-08-14, y sólo a medias.** El deploy ya no corre como superuser, así que la UBICACIÓN de E1b rige y el grado se mide en vez de declararse. Lo de arriba sigue valiendo para lo que importa: **la protección no está construida** — nada exige que un cambio protegido consuma una autorización, y `DROP POLICY p_app ON app.contact` como `crm_migrator` sigue funcionando.
...
`GET /api/contacts/:contactId/timeline` a través de `app.timeline_read`, y la región **Activity** en la ficha del contacto. Los cuatro estados: esqueletos (no spinners), un vacío que **enseña la primera acción** en vez de reportar ausencia, la lista, y el error **INLINE en la región y en ningún lado más** — la cabecera y el teléfono siguen usables, porque una vendedora tiene que poder llamar aunque el historial no cargue.

🔴 **EL CURSOR TRUNCABA A MILISEGUNDOS Y PERDÍA FILAS.** El cursor **es** ese string devuelto tal cual como `p_before_at`, así que formatear con `MS` convierte un ancla de `12:00:00.123456` en `12:00:00.123` y el predicado keyset `(occurred_at, id) < (before_at, before_id)` excluye **todas** las filas entre `.123000` y `.123456`. La página dos arranca tarde. **Nada de esto se ve en un camino feliz:** las filas están ordenadas bien y lo único mal es el **conteo**. 🎯 Mutación, sobre 55 entradas a un microsegundo de distancia: **51 de 60** recorridas, nueve desaparecidas. El fixture está construido a distancia sub-milisegundo justamente por eso — a una fila por segundo el cursor truncado pasa y la puerta no prueba nada.

🎯 **EL QUINTO VEREDICTO NO TENÍA FRASE.** `summaryKeyFor` nació como un `switch` de cuatro casos con `default`, así que `blocked_recording_unverified` —que la puerta **sí** devuelve en un estado all-party— se renderizaba como *"Not sent"* sin razón, contra el requisito de §11 de la razón en inglés llano. Ahora son dos literales `satisfies Record<…>`: un décimo `timeline_kind` o un séptimo `gate_verdict` **no compila** con *"missing property"*, `TimelineSummaryKey` se deriva de ellos, y el componente tipa su catálogo como un `Record` **TOTAL** de esa unión. El `?? entry.summaryKey` que le habría mostrado `gate.block.…` a una vendedora ya no existe porque no queda nada que atrapar.

⚠️ **64 VIOLACIONES SERIAS DE AXE ESTABAN SENTADAS EN UNA PANTALLA QUE SE ABRE TODO EL DÍA.** La suite de a11y sólo visitaba la forma **not-found** de `/contacts/:id`, así que la ficha real nunca había sido escaneada. El panel de Aloware envolvía pares `dt`/`dd` en `<span style="display:contents">` — que esconde el wrapper de la grilla pero **no del árbol de accesibilidad**, y HTML sólo admite `dt`, `dd`, `div`, `script` y `template` como hijos de un `dl`. Un `div` lo arregla y no cambia un píxel. **La lección no es el span: es que una puerta que sólo mira el estado de error de una URL no cubre la URL.**

⚠️ **Y EL TIMELINE ROMPIÓ EL FIXTURE E2E, como rompe cualquier FK nueva.** `removeCard` borraba el contacto entre la transacción del movimiento y el tick de un segundo del relay, así que el proyector llamaba a `timeline_upsert` para un contacto que ya no existía: la entrega reintentaba sus ocho intentos, para siempre, en cada corrida. Ahora **espera a que el outbox se asiente** — contando `claimed` además de `pending`, porque una fila reclamada es exactamente la que está en vuelo y la primera versión de esta espera devolvía cero justo en el momento **menos** seguro para borrar — y **tira error en vez de rendirse en silencio**. Borra `timeline_entry` junto con el contacto: una proyección es reconstruible, a diferencia del ledger, y su FK **queda estricta** en vez de ganar un `ON DELETE CASCADE` que nada dispararía — **`crm_app` no tiene DELETE sobre `app.contact`**, así que ningún camino de producción alcanza ese estado.

✅ **Verificado en pantalla, con datos reales por el camino real:** una venta por move-sheet de **$750/mes** se lee como **UNA** línea — *"Moved / to Closed Won · $9,000 / You / Today 9:44 AM"*. Ahí están la anualización ×12, el formato desde un string de centavos **sin aritmética en el cliente**, y el merge de `stage_changed` + `won` cayendo en una fila en vez de dos a un milisegundo. Un contacto sin historia renderiza el vacío.

**La pantalla busca sus propios datos** porque `ui.loader_whitelist` sanciona **un** loader SSR y ya existen tres: `AP005` rechaza un cuarto, y el motor forzó la arquitectura que §1.1 quería igual. Va **keyed en `contact.id`**, así que un segundo lead es una segunda instancia y no la historia de otra persona bajo este nombre.
...
**G3 — el camino del dinero. CERRADA.** La escalera la traía como *"mayormente"* desde hace semanas con una línea de prosa nombrando lo que faltaba.

- 🔴 **`idle_in_transaction_session_timeout` faltaba en DOS roles: `crm` y `crm_ci`.** El gate de cierre toma `FOR UPDATE` sobre la fila de la oportunidad y escribe la proyección del leaderboard en la misma transacción. Una sesión que abre una y se detiene —una laptop que se durmió, un editor que crasheó, una ventana de psql abierta durante el almuerzo— sostiene esos locks mientras viva la conexión: **cada vendedora que intente cerrar esa tarjeta espera detrás, y el tablero público deja de moverse, sin un solo error en ningún lado.** Y los dos que faltaban son justo **los que usa un humano**, que es de donde salen las transacciones ociosas. 5 min para `crm` (es el que corre migraciones; 15 s abortaría deploys) y 15 s para `crm_ci`.
- 🎯 **Y la aserción que la escalera arrastraba como "falta": matar el proceso a mitad del gate.** Ésa no se puede razonar — que un lock se libere al terminar el backend es una propiedad del servidor, no de nuestro código, y la única forma de saberlo es matar un backend que lo sostiene. El test abre la transacción, toma el mismo `FOR UPDATE`, **espera a que el lock exista de verdad** (si no, matar prueba nada), termina el backend y afirma que la fila vuelve a ser escribible con `FOR UPDATE NOWAIT`. **Mutación: suprimir el `pg_terminate_backend` → rojo.**
- ✅ **Las cuatro tablas append-only en un solo lugar, y como DUEÑO.** §2535(a) pide *"incluso desde la consola SQL del proveedor — que es lo que prueba el TRIGGER y no meramente el REVOKE"*. Un test como `crm_app` pasaría por el revoke solo y no diría nada del día que alguien restaure un grant. Incluye el `ON CONFLICT DO UPDATE` accidental, que es la forma realista: un upsert escrito por comodidad convierte un ledger append-only en mutable justo cuando alguien re-corre un import.
- ✅ **La atomicidad del gate, ahora sobre la cadena COMPLETA.** §2535(b) la lista terminando en *"event_emit + filas de outbox"* — y esos dos eslabones **no existían hasta la 0054**. O sea que ésta es la primera corrida de la aserción contra la cadena entera: falla un eslabón posterior al ledger y no sobrevive ni la transición, ni el evento, ni la fila de outbox, ni el movimiento de la tarjeta.
- ✅ **`annualize` exacto sobre 10⁵ valores bajo `fast-check`** (dependencia nueva, nombrada por el propio §2535(e)). El typecheck rechazó la primera versión porque pasaba bigints crudos: **`fromCents` es la única puerta al tipo branded**, y atravesarla es parte de la propiedad, no ceremonia.

**G5 — equivalencia plegado/separado. A UNA ASERCIÓN.**

- ✅ **(a) las cuatro configuraciones de `PROCESS_ROLES`**, no las dos que se usan — que es el punto: el camino separado se ejercita antes de que producción lo ejecute por primera vez.
- ✅ **(b) la unión de unidades por rol es exactamente el registro.** *"Una unidad que no pertenece a ningún rol, o a dos roles que ambos la corren, falla el test."* **Esta aserción no se podía escribir antes de la endpoint factory** — no había de qué tomar unión. Ahora cada ruta declara su `role` y `QUEUE_SPECS` declara las del worker, y el `ingest` tiene exactamente un miembro.
- ✅ **(e) el grep gate de las formas de URL tachadas**, y costó dos intentos, los dos instructivos. **Primero salió rojo por los COMENTARIOS que explican la regla** — `routes.ts` y `webhooks-aloware.ts` citan `/hooks/` para decir que está prohibido. Es el espejo exacto de una trampa ya pagada: el gate de definers una vez **pasó** porque un comentario mencionaba la llamada que buscaba. Mismo origen, signo opuesto, misma respuesta: despojar comentarios. **Después la mutación mostró que el gate seguía sin atrapar la forma tachada** cuando aparece al inicio de la cadena (`routes.ts` monta sin slash inicial): la atrapaba sólo el control positivo. **Un gate que necesita que otra aserción lo cubra no es el gate que dice ser.** Corregido haciendo de la comilla misma el límite.
- ⚠️ **(c) y (d) NO corridas, y por razones distintas.** (c) —la suite e2e idéntica contra las dos topologías, sin que ningún test sepa cuál corre— es pesada pero construible: son dos servidores y dos corridas de Playwright. **(d) no es construible acá**: `in.<domain>` resolviendo al servicio plegado y, tras sólo un repunte de CNAME, al de ingesta, necesita **DNS y dos servicios desplegados**. Se suma a G1, G4(c) y G9 en la lista de lo que espera la instancia de Render.
...
⚠️ **ESTO SALIÓ DE UNA AUDITORÍA, NO DE UN PLAN.** Un barrido de los 68 ítems del MVP contra el árbol dio **4 DONE · 50 PARTIAL · 13 MISSING · 2 BLOQUEADO**, y el hallazgo que lo atraviesa todo: **hay motor y no hay cableado.** `app.event_emit`, `app.compliance_check`, `app.consent_append`, `app.suppression_append`, `app.contact_edit`, `app.contact_flag_bad_number` y `app.schedule_meeting_reminder` estaban completas, endurecidas, probadas — y con **cero llamadores de producción**. `app.activity`, `app.meeting` y `app.opportunity` no tenían escritor ninguno. Es el modo de fallo firmado de `contact.last_touch_at`, ahora a escala de **tabla y de función**, no de columna.

🔴 **UN DEFECTO CON FECHA: EL 2026-08-25 NINGUNA VENTA COMMITEABA.** `event_outbox` va particionado por día, **sin partición DEFAULT a propósito**, y la 0043 llamó a `ensure_event_partitions` **una sola vez, adentro de sí misma** — horizonte hasta el 24 de agosto y nadie más la llamó. El `INSERT` del fan-out vive dentro de `app.event_emit`, que vive dentro de la transacción del emisor: pasado el 24, `23514` y rollback de **la transacción entera**. Cableado el primer emisor a `app.stage_move`, eso es la vendedora viendo que la tarjeta no se mueve. **Y no se podía arreglar llamándola:** shippeó `SECURITY INVOKER` y `crm_app` no tiene `CREATE` sobre el esquema `app` — cada llamada habría dado `permission denied`. Ahora es definer, **termina en `security.harden()`** (una partición creada en caliente nace sin RLS, que es el defecto que `silo.test.ts` ya cazó una vez) y se re-afirma **al boot y en el tick**, que es la única de las tres garantías de la constitución que aplica acá.

🔴 **UNA VENTA ESTABA POR ACREDITARSE DOS VECES, y nada en el árbol lo habría dicho.** ADR-002 calcula el fan-out con `WHERE ... delivery IN ('outbox','pgboss')`, y esa columna **no existía**, así que `('earnings','opportunity.won')` recibía fila de outbox mientras `app.stage_move` ya acredita el ledger inline (0019:257 la venta, :284 la reversa). El día que un relay tuviera handler de `earnings`, una venta = dos créditos, **permanentes** — el ledger es append-only y no tiene job de recomputo por diseño, y lo primero que se habría notado es el tablero público leyendo doble.
- **La clasificación se leyó del árbol, no se eligió:** `earnings` consume nueve eventos y **dos** son los que `stage_move` ya escribe. Esos dos pares son `inline`; el resto queda como estaba.
- ⚠️ **Los otros siete pares de `earnings` NO se decidieron acá.** No tienen escritor inline, así que `outbox` es la respuesta honesta hoy. Si alguno debe volverse inline es una decisión sobre dinero y es de Jorge.
- 🎯 Mutación: sacar el predicado → rojo nombrando a `earnings` presente en el fan-out.

✅ **`handler_built`: "declarado pero no construido" deja de ser un comentario.** El corpus (§826) manda que un consumidor sin handler rompa el build; aplicado literal hoy pone rojo en **17 lugares**, porque hay 19 consumidores declarados y 3 módulos. Se conservó la regla y se hizo honesto el momento: **un consumidor sin handler no recibe fila de outbox** —que es lo único cierto, porque una fila de outbox significa *"este consumidor todavía debe este evento"* y un módulo que no existe no debe nada— y **el evento se escribe igual y completo en `event_log`**, así que un consumidor construido el mes que viene se rellena desde el store. La alternativa (fallar la entrega) llenaba `dead_letter` con siete filas por evento diciendo *"no se llamó a un módulo que nunca se construyó"*, y la superficie hecha para mostrar lo que se rompió se habría llenado de ruido.
- 🎯 **El emparejamiento es un gate en las dos direcciones**, no un hábito: registrar un handler sin marcar la columna → rojo; marcar la columna sin handler → rojo. Probado por mutación.

✅ **`app.audit_log` — ítem 7 del MVP, la tercera tabla append-only, Y CON ESCRITOR DESDE EL PRIMER DÍA.** `append_only_tenant_admin`: `harden()` le da `SELECT` **y nada más** (verificado contra el motor), `WITH CHECK (false)`, y `security.refuse_mutation()` como trigger de sentencia que cubre `TRUNCATE`. Lectura **tenant-admin, no de dueño**: una vendedora que pudiera consultarla se enteraría de a qué colega le están mirando el libro.
- 🔴 **DOS COSAS QUE `05b` ESPECIFICA NO SE PUEDEN CREAR COMO ESTÁN ESCRITAS, y las dos se probaron en vivo.** (1) El índice único parcial del dedupe sobre el padre particionado lo rechaza Postgres 18 (*"must include all partitioning columns"*), y agregarle `occurred_at` destruiría el punto — va **por partición**, que es equivalente y no aproximado, porque `dedupe_bucket` es un truncado a 5 min y los límites de mes caen a las 00:00:00. (2) El `ON CONFLICT` correspondiente falla contra el padre; el escritor usa `WHERE NOT EXISTS` más `EXCEPTION WHEN unique_violation`, preservando lo que el corpus realmente pide: **bucketing por insert-o-nada, nunca por UPDATE**.
- **`actor_user_id` NO es parámetro** — sale de `app.current_user_id()`, mismo patrón que `consent_append`: un llamador que puede declarar quién actuó puede declarar que actuó otro.
- ⚠️ **De las 16 acciones, 5 son ratificadas y 11 DERIVADAS.** US-9.13 enumera once categorías que hay que auditar y no le pone nombre a ninguna. Nombrarlas es una propuesta que compila, en el mismo sentido en que el catálogo de eventos marca nueve payloads `"derived"`.

🔴 **EL GATE DE FANTASMAS ME ATRAPÓ DOS VECES Y TENÍA RAZÓN LAS DOS.** Escribí `lead.owner_changed` y `earnings.adjusted` como **acciones de auditoría**; las dos son **nombres de evento descartados**. El gate escanea literales y no puede distinguir una acción de un evento — y no debe: una acción escrita igual que un evento tachado es exactamente la ambigüedad que la lista de 33 fantasmas existe para impedir. **Y había cuatro colisiones más contra nombres VIVOS, que el gate de fantasmas no ve.** Renombradas las once derivadas, y `audit-vocabulary.test.ts` ahora lo assertea en las dos direcciones — contra vivos y contra fantasmas — con control positivo. Antes era algo que alguien verificó una vez.

✅ **EL RELAY, Y LA REGLA QUE LE DA LA FORMA: HANDLER Y ACK EN LA MISMA TRANSACCIÓN.** ADR-001 lo pide y es toda la diferencia entre exactly-once y at-least-once para un consumidor de sólo-base: si el handler commitea y el ack no, el lease vence y el trabajo corre de nuevo. Por eso **el handler recibe la transacción abierta y no puede abrir la suya** — la firma es el mecanismo.
- **No es una cola de pg-boss.** ADR-005 fija un piso de poll de 1 segundo y el piso de cron es un minuto. El outbox es dueño del fan-out, pg-boss del scheduling; darle el relay a pg-boss pondría a los dos a creer que entregan.
- 🔴 **EL RELAY NO PODÍA LEER EL CUERPO DEL EVENTO, medido y no supuesto.** `event_log` es `append_only_owner`, y `begin_system_work` pone `user_id` en `''` y scope en `'system'`, mientras `scope_is_global()` sólo contesta true para `tenant_read`/`tenant_admin`. Un relay en Drizzle habría leído **cero filas** y reportado *"evento no encontrado"* en cada entrega, para siempre, con cara de cola vacía. `app.outbox_payload()` es un definer que acota **por tenant y no por usuario**, que es la misma forma que `app.scheduled_job_resolve` ya usaba por la misma razón.
- **El claim devuelve `created_day`** (cinco columnas, no cuatro) porque es parte de la PK y sin él el ack no puede nombrar la fila. Sigue **sin payload**, y el test lo assertea nombrando las columnas prohibidas en vez de sólo contar.
- **`outbox_fail` convierte `attempts` en escalera.** El claim ya incrementaba y nadie movía `next_attempt_at`: un consumidor que reventaba se re-reclamaba en cada vuelta — a 1 tick/segundo, 3.600 intentos por hora sobre la misma fila. La escalera sale de la **columna del consumidor** (ADR-009), nunca de una constante: una proyección y un SMS a un lead no pueden compartirla.

⚠️ **LO QUE ESTE BLOQUE NO HIZO, dicho y no insinuado:** falta el **primer emisor real** — `app.stage_move` todavía no llama a `app.event_emit`, así que el outbox sigue vacío en producción y `audit_log` sólo recibe filas en los tests. **Y el emisor no es una línea:** de los cuatro payloads candidatos, `opportunity.reopened` es **el único que `stage_move` puede llenar completo** hoy; `won` declara `source_channel`, `vendor_id`, `days_from_lead_created` y `touches_to_close`, que no existen como columnas, y `stage_changed` declara `required_fields_satisfied`, que nada en el árbol produce. Eso es un diseño, no un cableado.

⚠️ **Y LA CADENA DE SNAPSHOTS ESTÁ MÁS DERIVADA DE LO QUE ESTE REGISTRO DECÍA.** `0050_snapshot.json` no conoce `event_consumer`, `event_log`, `event_outbox`, `consent_ledger` ni `suppression_list` — le falta todo el rango 0042–0043. Los snapshots siguen el contenido de la migración que lleva su número y nada más. 0051–0053 continúan la convención con los ids bumpeados; **no la reparan.**

📌 **Y una corrección de método al gate de índices de `snapshot-chain`:** reportaba un índice llamado `IF`. Un índice creado por `format()` se lee `CREATE UNIQUE INDEX IF NOT EXISTS %I ON ...`, `%` no está en la clase del nombre, y el motor backtrackeaba y capturaba `IF`. Exigir `ON` después del nombre lo rechaza y —el punto— saltea la forma dinámica, que es **el mismo trato que el chequeo de tablas ya le da a una partición de `format()`** y por la misma razón.
...
| `virtualizacion-tablero` | ~~**0043** · **0044**~~ → **RESUELTO al integrar (2026-08-10): 0045** leaderboard_read_hoists_its_clock · **0046** leaderboard_poll_budget |
...
`tests/integration/fixtures/perf-floor.ts` · `leaderboard-poll-perf.test.ts` · migraciones **0045** y **0046** · `perf-budgets.json`. **270 → 274 tests.**
...
- **La 0045 iza el corte a un CTE `MATERIALIZED`.** Medido después, mismo fixture, misma sesión: **177 ms → 0,87 ms · 13.000 buffers → 43**. De punta a punta P11 pasa de **238,6 ms a 14,6 ms p95** — adentro del **warn**, no apenas adentro del rojo.
...
- **La 0046 registra el presupuesto en 80 — el número RATIFICADO, no el medido.** A diferencia de P12, P13, P20 y N13, esta cifra se firmó en Fase 4 y el trabajo de la medición era comprobar si el producto estaba adentro, no fijar la vara. Registrar 14,6 habría inventado una regla más estricta que la firmada, con una sola máquina de evidencia.
...
### ✅ ~~COLISIÓN DE MIGRACIONES ENTRE WORKTREES~~ — RESUELTA AL INTEGRAR (2026-08-09, cerrada el 2026-08-10)
...
**CERRADA.** `master` entró en esta rama el 2026-08-10: las migraciones son **0045** y **0046**, el journal continúa desde el `0044_contact_editing` de master y los snapshots encadenan desde el suyo. La otra sesión construyó el mecanismo que faltaba mientras tanto — `DBGEN004` y `migration-index.test.ts`, documentados arriba — así que esto no puede volver a pasar en silencio. Lo que sigue abajo es el registro de cómo se veía desde acá, que vale conservar porque el síntoma fue engañoso.
...
**El ítem 1 de la lista de "lo que no depende de nadie" queda cerrado.** El chip pasa de `NEW 06m` congelado a `NEW 04:12` avanzando, que es lo que §2.7 dibuja y lo que §4.2 registra como `board.card.new_clock`. **P6 con los relojes corriendo: 16,8 ms, cero long tasks** — contra 116,7 ms del primer intento y 50,0 ms del segundo. No cambió el enfoque: cambió la premisa. La virtualización dejó ~28 chips en pantalla en vez de 500, y verificado a mano avanzó **4 segundos en 4 segundos de reloj de pared**.

- **Un solo intervalo para todo el tablero**, con `Set` y no arreglo — la lección O(n²) que este mismo registro anotó cuando el patrón de `Notification.permission` (un suscriptor) se aplicó a quinientos.
- **Sólo el chip que tictaquea se suscribe.** `SignalChip` se parte en dos componentes en vez de tener un hook condicional: suscribir a todos re-renderizaría cada `Due 3 h ago` una vez por segundo para redibujar una string medida en horas.
- **El reloj NO lee ningún reloj de pared.** El servidor manda la edad que él midió y el navegador le suma su propio `performance.now()`. Una máquina tres minutos adelantada mostraría `NEW 03:00` sobre un lead recién llegado, y el ruling **R** llama a speed-to-lead *"el único número que justifica el gasto en leads"* — o sea el último al que le está permitido ser aproximadamente correcto.
- **Vence solo a los sesenta minutos** y el slot colapsa. Un tablero abierto toda la mañana es el caso normal, no el borde: sin esto el chip seguiría contando hasta `NEW 61:04`, un estado que el servidor dice que terminó. El cliente no puede computar qué lo reemplaza —§2.8 pone esa precedencia en el servidor a propósito— así que la tarjeta queda sin señal hasta el próximo loader: **incompleto, no equivocado.**
- **`app/lib/board/tick.ts`, no al lado de la derivación que sirve.** El intento anterior exportó su `mmss` desde `app/routes/api/board.ts` y arrastró postgres al bundle del navegador. La puerta que ese fracaso pagó —`client-server-boundary.test.ts`— es exactamente la que hace seguro este intento.

🔴 **EL FIXTURE HABÍA DEJADO DE CUBRIR ESTO EN SILENCIO, y lo encontré antes de correr P6 y no después.** `perf-500` es persistente y `seedPerf500` sale temprano si ya hay 500 filas, así que `created_at` se quedó con el valor de la primera siembra. **Una hora después, ninguna de las 500 tarjetas era `fresh`** — el chip no renderizaba en ninguna, y P6 habría medido un tablero **sin un solo reloj**. La puerta que rechazó esta feature dos veces habría pasado en verde **sin medirla**, y nada lo habría dicho.

- **Cerrado por los dos lados:** el fixture reestampa las llegadas en cada corrida (las 500 frescas es su peor caso documentado — un vendedor que acaba de importar su libro tiene exactamente eso), y `drag-perf.spec.ts` **cuenta los relojes en pantalla mientras mide** y se pone rojo si son cero. 🎯 **Probado por mutación:** con la ventana fresca en 0 el gate falla nombrando *"no NEW clock was on screen, so this run did not measure them"*.

🔴 **UNA TRAMPA DE PRECEDENCIA QUE NO PUEDO ARREGLAR TODAVÍA, anotada donde vive la puerta.** El gate del chip es `attempts === 0`, y **no es el que el corpus ratifica.** §2.7 lo condiciona a `first_touch_at IS NULL`, y el ruling **R** es explícito: *"el reloj sigue corriendo hasta que `call.completed` llegue con resultado `connected` o `voicemail`"*, y §2.7 deletrea el caso que esto rompe — *"un intento sin respuesta incrementa attempts y el reloj sigue corriendo"*. Hoy una vendedora que disca y no le atienden **pierde el chip y no debería**. La razón no es un atajo: **no existe la columna `first_touch_at`** — `opportunity.first_touch_latency_seconds` es una duración derivada, no el timestamp que la puerta necesita, y nada puede escribir ninguna de las dos hasta `call.completed`, que es Aloware, que es la Puerta 2.

⚠️ **Mi propio test nació con fecha de vencimiento y se puso rojo en la primera corrida.** Fijé el valor inicial exacto (`NEW 04:10` sobre una tarjeta retrofechada 4:10) y el sign-in tarda algo más de un segundo, así que arrancó en `04:11`. **El número que importa no es dónde ARRANCA el reloj —eso depende de qué tan rápido loguea la máquina— sino cuánto se MUEVE.** Reescrito para leer el valor y assertear el avance. Las tarjetas las crea el test y las edades son relativas a `clock_timestamp()`, con un helper `backdateArrival` nuevo: es la misma trampa que puso rojo a `card-anatomy` dos veces, evitada a propósito esta vez.

- **P20 quedó en 2181 ms con TBT en 0** — el intervalo no costó nada medible. P12 y P13 sin cambios apreciables.

### 🪟 EL TABLERO VIRTUALIZA, Y UNA ASERCIÓN DE P6 TUVO QUE CAMBIAR (2026-08-09)
`app/lib/board/virtual-window.ts` · `pipeline-columns.tsx` · `shell.tsx` · `board.tsx` · `theme.css` · `reset.css` · `tests/e2e/board-virtualization.spec.ts` · `tests/integration/virtual-window.test.ts`. **243 → 258 tests · 83 → 87 e2e.** Sin migración.

**P6 PASA DE 33,3–50,0 ms A 16,8 ms, con long task 0,0.** El ítem que la entrada de la Puerta 11 dejó anotado —*"la mediana pasa con 0,6 ms de margen y una de cada tres corridas se pasa"*— queda cerrado, y con el margen que decía que sólo la virtualización iba a dar: **17 ms contra un presupuesto de 34**. El frame quedó apoyado en el piso de 60 fps del compositor, o sea que el drag ya no cuesta nada medible.

- **P20 bajó de 2251 a 2176 ms y NO lo ratcheteé.** El presupuesto sigue en 2300. Mover un número dependiente de la máquina para ganar un 3% compra poco y cuesta robustez, y la propia fila dice que re-baselinear es un acto deliberado.
- **P12 subió de 108.086 a 112.120 bytes** (88% del presupuesto). Ése es el costo del virtualizador, escrito a mano: el corpus no nombra ninguna librería y no hacía falta ninguna.
- ⚠️ **LA VIRTUALIZACIÓN CORTÓ EL DOM, NO EL PAYLOAD, y por eso TTI casi no se movió.** El HTML del servidor pasó de 125 tarjetas por columna a 10, pero el loader sigue mandando las 500 como datos de hidratación. **`04b` §3.6 P1 dice *"the server never returns 500 cards"* y `board.ts` sigue sin `LIMIT`** — ARR-UX-07 (20 por columna) es la otra mitad del contrato y quedó fuera a propósito: el cursor para las tarjetas 21+ no está especificado en ningún documento, hay que reconciliarlo con los deltas de 15 s, y `Load 25 more` de Fase 2 no tiene clave de string ratificada en la tabla `board.*` de §4.2, así que hoy no puede renderizarse sin romper §7.3.

🔴 **LA ASERCIÓN DE ENGAGEMENT DE P6 CAMBIÓ, y lo digo primero porque desde un diff es indistinguible del fallo que todo este diseño existe para prevenir.** `drag-perf.spec.ts` asserteaba `rendered === 500` — *"500 cards must be mounted while measuring"*. La escribí yo para tapar un agujero real: un presupuesto de frames en verde comprado midiendo un tablero que no estaba. Pero expresó *"el tablero entero"* como **un conteo de nodos del DOM**, y `04b` §9 **R2-2** hace de ese mismo conteo un **fallo de build** arriba de catorce por columna. La aserción y la especificación querían cosas opuestas; la aserción se escribió segunda, en una sesión en la que la virtualización no existía.

- **La propiedad se mudó, y a un lugar más fuerte.** `rendered === 500` probaba una cosa: que el tablero bajo el drag era el tablero entero. Ahora lo prueban tres — el fixture sigue teniendo 500, las columnas **reportan** 500 con conteos calculados en el servidor, y **scrollear alcanza la tarjeta 499**. La tercera es la que `rendered === 500` no podía hacer nunca, porque un tablero que monta todo no puede perder nada. Uno virtualizado sí, y perder una tarjeta es una vendedora perdiendo un lead: **una columna que termina una tarjeta antes se ve exactamente igual que una columna con una tarjeta menos.**
- **Y ahora se assertea la inversa:** montar más de 14 es rojo, **durante el drag** y no sólo en una página quieta. La puerta vieja habría pasado en verde justo la regresión que ésta existe para atrapar.

🔴 **EL PITCH QUE §2.1 PUBLICA ESTÁ MAL, y lo copié casi.** §2.1 dice `topOfCard(i) = i × 120`. Esos 120 son el pitch de una tarjeta de **112 px** — la **tercera** altura que `04b` cargó, y la que el hallazgo **C11 nunca miró**: C11 sólo enfrentó los 108/92 de §3.6 contra los 120/156 de §1, así que N17 tachó 108 y dejó 112 en pie. Con N17 vigente el pitch real es **128 desktop / 164 mobile** (que es lo que §4.1 dice, *"row stride = 128 px"*). Copiar §2.1 habría corrido cada tarjeta 8 px, acumulando: la 40 a 320 px de donde el scroll container la cree.

- **`--card-pitch` es token nuevo y `--card-gap` se DERIVA de él** (`calc(--card-pitch - --card-h)`). Las dos rutas —DOM plano abajo de 30 tarjetas, offsets absolutos arriba— leen un solo número, así que no pueden terminar en dos ritmos. La altura ya está `pinned` en `ref.ci_ratchet`, y `card-height.test.ts` assertea que la **diferencia** son 8 px, así que el pitch queda anclado sin fila propia ni migración.
- 🔬 **Probado por mutación con el número exacto de §2.1.** Poner 128 → 120 pone **rojo** `card-height.test.ts` nombrando los 8 px que faltan. **Y el e2e de pitch quedó VERDE**, anotado en el propio spec: ese test prueba que el layout y la aritmética coinciden **entre sí**, y a 120 siguen coincidiendo — las tarjetas simplemente quedan pegadas. Dos tests, dos preguntas distintas, ninguno redundante.

🔴 **UN BUG QUE ENCONTRÓ EL TEST UNITARIO Y QUE SE ALCANZA SIN OVERSCROLL: la columna en blanco.** La primera versión no acotaba `first` al final de la columna, sólo `end`. Con un offset pasado del contenido, `first` quedaba más allá del último índice y el conteo en cero. La ruta real no es el rebote de iOS: **arrastrá una tarjeta fuera de una columna scrolleada hasta el fondo** y el update optimista encoge el contenido bajo un offset retenido. Todas las tarjetas presentes, ninguna renderizada.

🔴 **Y UNO QUE SÓLO APARECIÓ MIDIENDO LA PANTALLA: `min-height` no acota.** Puse `minHeight: 100dvh` en el envoltorio del shell, que se lee bien y está mal — el envoltorio igual crece con su contenido, así que `flex: 1` le entregó al tablero la altura **crecida**. Medido en la página real: **el scroll container de cada columna medía 16.008 px**, `scrollHeight - clientHeight` era cero, y nada scrolleaba. Cada tarjeta posicionada bien, la aritmética bien, y el tablero renderizando diez tarjetas y parando. **Un virtualizador que nunca recibe un evento de scroll se ve exactamente igual que una columna con diez tarjetas adentro.**

⚠️ **EL MODELO DE SCROLL DEL TABLERO NO EXISTE EN EL CORPUS, y hubo que elegirlo.** Cero `overflow-y`, cero altura de columna, cero `100vh` en 1,9 MB. Toda la evidencia es indirecta y apunta a kanban clásico —§2.1 hace su aritmética contra un *"900px column viewport"*, el header de columna es sticky, §3.3 dice *"board keeps its scroll position"*, §2.6 permite scroll 2-D en el tablero como excepción explícita— así que cada columna es ahora su propio contenedor acotado. **Es una decisión tomada, no heredada.** El autoscroll de drag sigue declarado **sólo horizontal** en §1.5; el vertical dentro de una columna no está especificado y no lo inventé.

🔴 **AXE ATRAPÓ UNA REGRESIÓN DE ACCESIBILIDAD QUE CAUSÉ, en My Day a ancho de teléfono.** Sacar el scroll del documento y ponerlo en un `div` produjo un `scrollable-region-focusable` **serious**: una región que el mouse scrollea y el teclado no alcanza. El documento nunca tuvo el problema porque los navegadores lo scrollean gratis — es justo lo que se pierde al quitarle el scroll.

- **El remedio es un tab stop, y uno incondicional habría sido el segundo defecto:** caería antes de la barra de undo en toda pantalla, y `undo-keyboard.spec.ts` acota cuántas presiones puede quedar atrás precisamente porque la barra vive cinco segundos. **La región es focusable sólo cuando de verdad scrollea.** El tablero no scrollea ahí, así que en la pantalla donde una presión desperdiciada cuesta más no hay nada que operar y no hay stop. Verificado: el undo sigue en **#5 de 16**.

⚠️ **R2-2 dice *"at any viewport"* y la aritmética no puede darlo.** La ventana es aproximadamente `viewportH / pitch + 3`, así que 14 aguanta hasta un viewport de columna de **1410 px** y se pasa arriba de eso — un monitor 4K al 100% llega. Las dos salidas son peores que el desborde: acotar la ventana deja una banda visible de columna vacía, y subir el número es aflojar un presupuesto. **El test fija los 1410** para que el día que se ponga rojo el diagnóstico ya esté escrito.

⚠️ **DOS TESTS DE `card-anatomy` FALLAN Y NO ES ESTO — probado por bisección, no supuesto.** `git stash` y los mismos dos fallan idénticos en HEAD limpio: Curtis pide `Going cold · 9d` y el seed ya lleva **13d**, y el riel de Alma llegó al 100%. Es exactamente la clase de test con fecha de vencimiento que esta misma sección ya registra para Ruth, del otro lado: **el seed estampa edades absolutas y las aserciones son relativas.** Se arregla re-sembrando, y `db:seed` se niega a correr dos veces sobre el mismo ledger — hace falta `db:reset`, que es de Jorge.

- ⚠️ **Y con 13d en vez de 9d la tarjeta de Curtis se CLIPEA**: el chip más ancho hace que la fila del dinero envuelva a dos líneas y `Move` queda cortado abajo. Preexistente, no de este cambio, pero deja ver que la anatomía está a un carácter del borde de los 120 px. `05c` §10.7 B7(b) ya declaró qué se hace si eso pasa: **se corta la anatomía, no la altura.**

⚠️ **Nota operativa del worktree:** no tenía `.env` ni `node_modules` propios, y Vite se niega a servir `@fs/` fuera del root — así que la página **no hidrataba** y los e2e pasaban por las razones equivocadas (un `.focus()` y una medición de SSR pasan sin hidratación). Se resolvió con `npm ci` adentro del worktree. **Vale reconocerlo rápido: si `data-drag` no aparece, no es el código.**
...
| — | **Escritores de aplicación** | 🟡 **NUEVO, y era el hueco más grande del producto (2026-08-12).** Hasta hoy la única tabla que una ruta escribía era `app.contact`. Ya se puede **empezar un negocio** (0065) y **escribir una nota** (0066), las dos por una puerta definer que emite en la misma transacción. **Falta `app.meeting`** —bloqueada por una decisión de contrato, no por código— y **`app.app_user`**, o sea que el producto no puede dar de alta ni promover a nadie |
...
> ⚠️ **Reconciliada el 2026-08-12.** Esta tabla decía que las Puertas 3, 5, 6 y 8 estaban sin empezar o a medias mientras el detalle de *Current State* decía lo contrario — **el mismo fallo que la nota del 2026-08-02 arriba dice haber arreglado, repetido en la sección de al lado**. Una sesión nueva lee esta tabla primero y planificaba sobre ella. Las filas de abajo llevan la fecha de su última verificación; **si una fila no tiene fecha, no fue verificada en esta pasada.**
...
| 5 | Equivalencia plegado/separado | 🟡 **el pliegue EXISTE, corrió, y su borde más filoso está probado.** El worker arranca dentro del proceso web (`app/jobs/boot.ts`) y produjo **las mismas dos filas terminales** que el proceso separado — `skipped: sms_disabled` y `dropped: 40m late` — y desde el 2026-08-09 el **contexto compartido en la conexión** que sólo el pliegue produce está cerrado (`folded-handoff.test.ts`, ver la Puerta 4). **Y desde el 2026-08-11 la (c) está cerrada:** la suite E2E idéntica corre contra las dos topologías y `gate-5-topology.test.ts` asserta que **ningún archivo bajo `tests/e2e/` menciona siquiera `E2E_TOPOLOGY`** — una suite que puede ver en qué topología está es una suite que puede acomodarse a una. Falta lo que da nombre a la puerta: equivalencia **bajo carga**, no en un caso feliz (2026-08-11) |
| 6 | Tormenta de 20.000 webhooks | 🔴 **NO SE PUEDE CORRER, y sus dos patas están trabadas por cosas distintas (2026-08-09).** La **tormenta** necesita el módulo Aloware entero — no hay ruta de ingesta, ni `inbound_webhook_event`, ni vault/dead_letter, ni el merge de `call.enriched`, ni 429, ni `folded_topology_saturated` — y la cadencia de **P24 dice literalmente *"Gate 2, then quarterly"***, o sea que el corpus la ancla a la Puerta 2. El **piso de polling** no depende de nadie pero todavía no es medible: faltan los dos pollers de UI y mover el del leaderboard al camino condicional. **Avanzado:** las tres rutas API responden conditional GET con una sola implementación y 11 e2e, los dos brazos, probado por mutación. **Y desde el 2026-08-11 el transporte de eventos existe entero** —`event_emit`, outbox particionado, relay con lease y ack en la misma transacción, `folded_topology_saturated`— así que la pata de la tormenta que dependía de eso ya no depende (2026-08-11) |
| 7 | SSE detrás del proxy | ⬜ no empezado |
| 8 | pg-boss bajo estrés de versión | 🟢 **CORRIDA (2026-08-11).** `gate-8-jobs.test.ts`: el 100% de la superficie de pg-boss está envuelta en `app/jobs/boss.ts` —es el único módulo que lo importa, y una regla de ESLint lo hace cumplir en vez de pedirlo—, la reconciliación de colas es idempotente y detecta deriva de política, y el dead-letter se probó por mutación. **Falta:** el salto de versión real, que es un ejercicio de upgrade y no de código |
...
1. ~~**El reloj `NEW` que tictaquea**~~ — **HECHO (2026-08-09).** Fue exactamente lo que decía esta línea: con virtualización el problema se disolvió solo. **16,8 ms con los relojes corriendo**, contra 116,7 y 50,0 de los dos intentos anteriores. Lo que queda abierto de este tema es la puerta del chip: `attempts === 0` no es lo que ratifica el ruling **R**, y la columna que hace falta (`first_touch_at`) llega con Aloware.
...
| 7 | Leaderboard público | ✅ tablero, undo honrado **y celebración** |
| 8 | My Day | ✅ |
| 9 | Aloware | 🟡 **EN CURSO.** ⚠️ **Esta fila estuvo DESACTUALIZADA desde el 2026-08-10 y se corrigió el 2026-08-12** — listaba como faltantes la ruta del borde, la tabla `call`, el job de merge y `message-merge`, **y los cuatro habían aterrizado el 2026-08-10 en `11c94d7`**. Una sesión leyó esta fila y planificó construir lo que ya existía. Hecho: el candado de tipo `alowareCapability()`, la evidencia de capacidad entrante (0031), la aserción de boot (0032), el mapeo de números (0033), las dos tablas de ingest (0034), el extractor del borde, `POST /api/calls` hasta `no_credentials`, `app.webhook_ingest()` (0035), **la ruta del borde `/webhooks/aloware/v1/:endpointToken` con 13 tests, la tabla `call`, `call-merge` y `message-merge` con sus dos colas**, y desde el 2026-08-12 **el escritor de credenciales (0068)** —sin el cual todo lo anterior contestaba 401— y **el sniff de STOP (0069)**. Falta: las líneas de latencia de `ref.job_registry` (`05c` §11.7), que es la mitad que le queda a G6/P24. **Sigue bloqueando producción `call_list`** — decisión abierta de Jorge |
| — | **Escritores de aplicación** | 🟡 **NUEVO, y era el hueco más grande del producto (2026-08-12).** Hasta hoy la única tabla que una ruta escribía era `app.contact`. Ya se puede **empezar un negocio** (0065) y **escribir una nota** (0066), las dos por una puerta definer que emite en la misma transacción. **Falta `app.meeting`** —bloqueada por una decisión de contrato, no por código— y **`app.app_user`**, o sea que el producto no puede dar de alta ni promover a nadie |
| 10 | Datos demo | 🟡 siembra por el camino real, se niega a duplicarse, `lost_reason` sembrados, **y desde el 2026-08-03 ABARCA LOS CUATRO PERÍODOS con una reversa** — las dos exigencias de §1529 que el registro nunca había anotado como faltantes. `DEMO-01..10` sigue siendo un registro verificado por máquina: **3 cubiertos, 4 bloqueados por Aloware/búsqueda, 3 parciales**, y los tres parciales encogieron |
...
| 3 | Camino del dinero | ✅ mayormente — append-only por trigger de sentencia (incluye TRUNCATE), exactly-once, transacción del gate atómica. **Y desde el 2026-08-11 la atomicidad se assertea sobre la cadena COMPLETA** —gate → transición → escritura de etapa → ledger → proyección → `event_emit` + filas de outbox—, que es la primera vez: los dos últimos eslabones no existían hasta la 0054. **Falta:** proceso muerto a mitad del gate sin dejar lock (2026-08-11) |
| 4 | Silo de punta a punta | 🟢 **CERRADA (2026-08-13).** ⚠️ Esta fila decía que faltaba el contexto heredado entre job y request en la misma conexión pooleada; **eso se cerró el 2026-08-09** con `folded-handoff.test.ts` — las dos direcciones, el job que revienta y el claim cross-tenant, todas exigiendo el mismo `pg_backend_pid()`, probadas por mutación. La fila quedó nueve días desactualizada. **(a)** cerrado (se niega a arrancar si el usuario puede saltear RLS) · **identidad sellada** (0067) · **tenant sellado** (0074, R1) · el resto lo cubre la suite de silo. **Lo que NO cierra: la propiedad (b)** — un ancla fuera del árbol de trabajo. ⚠️ **Actualizado 2026-08-14:** la *ubicación* de E1b ya rige (el deploy corre como `crm_migrator`, que no puede autorizarse a sí mismo) y el grado se **mide**; la *protección* sigue sin construirse |

...
| 6 | Tormenta de 20.000 webhooks | 🟢 **CORRIDA (2026-08-10), NO cerrada.** ⚠️ **Esta fila decía "no empezado ⬜" y era falsa** — la reconciliación del 2026-08-12 arregló las Puertas 3, 5 y 8 y se saltó ésta. Medido: **20.000 entregas a 333/s, 20.000 respondidas, CERO perdidas**, p95 de ingesta 12,41 ms, piso de polling 12,25 ms durante la tormenta, verificado contra la base (10.000 filas, 10.000 jobs, 0 dead letters). **No cierra** porque tres de sus cuatro aserciones sin sujeto ya lo tienen (semáforo, monitor de event loop, `folded_topology_saturated` — todas del 2026-08-10) y **queda una: G6/P24, la PROTEGIDA** — inyectar un STOP durante la tormenta. No hay sniff de STOP en el ingreso y `message.received` es inalcanzable |
| 7 | SSE detrás del proxy | ⬜ no empezado |
| 8 | pg-boss bajo estrés de versión | 🟢 **CORRIDA (2026-08-11).** `gate-8-jobs.test.ts`: el 100% de la superficie de pg-boss está envuelta en `app/jobs/boss.ts` —es el único módulo que lo importa, y una regla de ESLint lo hace cumplir en vez de pedirlo—, la reconciliación de colas es idempotente y detecta deriva de política, y el dead-letter se probó por mutación. **Falta:** el salto de versión real, que es un ejercicio de upgrade y no de código |
| 9 | Simulacro de restauración | ⬜ no empezado |
| 10 | Los 5000 ms en cuatro representaciones | ✅ **CERRADA.** Las cuatro existen —TS · CSS · predicado SQL de la proyección · claim de la celebración— y el test de deriva compara **valores, nunca nombres**, incluida la aserción de que `celebrate_once` no menciona `projection_reveal_delay_ms` |
...
**Al 2026-08-12: 731 tests · 122 e2e · `verify` verde.** Los puntos 1, 2, 3, 4 y 6 de la lista de abajo están cerrados, y también el riel de salud que la abría (ver las entradas del 2026-08-03).

**Lo primero de todo, y no depende de nadie: el SNIFF DE STOP EN EL INGRESO.** Es lo único que le queda a **G6/P24**, la aserción que el registro marca protegida con `retries: 0`; §10.9 lo ubica en `stopSniff`, que no existe. Hoy `message.received` es inalcanzable, así que ninguna entrega puede producir un STOP — y eso es exposición legal (TCPA), no una casilla de puerta. Con la credencial de la 0068 ya emitible, el camino entero está disponible por primera vez.

...
### 🎨 UNA PASADA DE DISEÑO, Y DOS DEFECTOS QUE SÓLO SE VEN MIRANDO LA PANTALLA (2026-08-04)
`shell.tsx` · `pipeline-columns.tsx` · `rank-and-gap.tsx` · `theme.css` · `primitives.css` · `reset.css`. **Sin tests nuevos · 88 e2e verdes · P12 +52 bytes · P13 2.462 → 2.739.** Nada de esto se decidió leyendo un documento: se decidió sacando capturas de las tres pantallas en los dos perfiles y mirándolas.

**LO QUE `04b` PUBLICABA Y EL ÁRBOL NUNCA IMPLEMENTÓ.** Tres escalas enteras existían sólo en el documento: **elevación** (§4.6), **z-index** (§4.7) y el **ancho de columna** (§4.5). Por eso cada tarjeta del producto era un contorno de 1px sobre un lienzo casi blanco — nada estaba mal, y nada tenía profundidad tampoco. Ahora la tarjeta lleva `--elev-1` en reposo y `--elev-2` al pasar el mouse, **sin transición**: §4.6 dice que la elevación es un token de estructura y que animar `box-shadow` pinta en cada frame, que es exactamente el presupuesto que mide la Puerta 12.

- 🔴 **DOS ORÍGENES IZQUIERDOS EN LA MISMA PANTALLA.** La barra superior estaba encajonada a 64rem, así que el logo caía a 230px del borde mientras el título `Pipeline` y su primera columna arrancaban en el margen de la página. En My Day y Earnings coincidía —son columnas de lectura centradas— y en el tablero se veía descosido. La barra pasa a **ancho completo**, que además es lo que es la barra de una aplicación; una centrada se lee como una landing.
- 🔴 **LA COLUMNA DEL KANBAN NO EXISTÍA EN PANTALLA.** Usaba `--color-surface-2` (n-25), **un escalón del lienzo n-50**, así que el tablero eran tarjetas flotando bajo una etiqueta. `--color-surface-3` (n-100) ya estaba en el archivo, sin usar, y el comentario del primitivo dice literalmente *"kanban column background"*. **No hizo falta ningún token nuevo: el correcto ya estaba.**
- **La cabecera de columna es `sticky`**, que C-32 pedía y que recién hoy importa: una columna puede tener 125 tarjetas, y llegar a la noventa te dejaba sin saber en qué columna estabas ni cuánto valía.
- **El ancho de columna era `17rem` cableado inline** — 272px contra los 288 publicados. Ahora es token, y **debajo del breakpoint de densidad se ensancha**: a 272px en un teléfono la siguiente columna quedaba **cortada por la mitad de una tarjeta**, y media tarjeta se lee como un fallo de renderizado, no como *"hay más a la derecha"*.
- **La tarjeta reparte sus filas (`space-between`) en vez de apilarlas arriba.** En escritorio los 120px no dejan sobrante y no cambia nada; en los 156px del teléfono **un tercio de la tarjeta estaba vacío debajo de la última línea**, que es lo que hacía que se viera sin terminar.

🔴 **EL GATE DE AXE SE PUSO ROJO LA MISMA HORA QUE LLEGÓ EL TINTE, y ése es todo su valor.** `color-contrast`, `impact: serious`, un nodo: el `p` de *"Nothing here yet."*. **`--color-text-tertiary` mide 4,90 contra `--color-surface-1` y 4,29 sobre la columna** — debajo de AA. La matriz de contraste de §5 está medida contra `surface-1`, así que **mover una superficie obliga a volver a elegir el escalón de texto, no a arrastrar el viejo**: la rampa no tiene margen de sobra por construcción. Pasa a `secondary` (6,13). El conteo de la cabecera tenía el mismo defecto y axe **no** lo nombró; se corrigió igual, porque el hallazgo es la clase y no el nodo.

🔴 **EL `·` COLGANDO VOLVIÓ, UNA CLÁUSULA A LA IZQUIERDA, CON SU TEST EN VERDE.** La regla de `reset.css` decía *"el ÚLTIMO separador"* y eso **sólo era cierto por casualidad**: se escribió cuando la frase tenía una línea menos, así que el punto final era el único huérfano. La frase creció, el salto de línea se movió, y el huérfano pasó a ser el **primer** punto — con `rank-and-gap.spec` verde, porque asserteaba *el punto que estaba huérfano esa semana* en vez de la regla. Ahora la clase se llama `standing-separator`, va en los dos, y el patrón del test quita los dos debajo del breakpoint. **Encontrado mirando el teléfono.**

- **`--color-overlay` era el único literal de color fuera de `primitives.css`**, en el archivo cuyo encabezado declara ser el único que puede tenerlos. Ahora se arma desde `--p-shadow-rgb`, que es el mismo ancla `#0b0e13` en canales porque un hex no puede llevar alfa. ⚠️ **Y esa regla sigue siendo documentación, no mecanismo:** no existe ningún test que la haga cumplir.
- 📐 **P6 vuelto a medir con las sombras encima: 16,8 ms en las TRES corridas** (antes de esta pasada, 33,3 / 16,8 / 16,8). **P20: 2217 ms.** Las sombras no costaron nada medible.

### 🪟 LA COLUMNA SE VIRTUALIZA: 500 NODOS PASAN A 52, Y P6 DEJA DE ESTAR AL BORDE (2026-08-04)
`app/components/board/column-window.ts` · `app/styles/tokens/geometry.ts` · `pipeline-columns.tsx` · `tests/e2e/column-virtualization.spec.ts`. **243 → 245 tests · 57 → 61 e2e por perfil.** Sin migración: no toca la base.

**P6 PASA DE 33,3 ms A 16,8 ms.** Contra un presupuesto de 34, la mediana de tres corridas era 33,3 con *una de cada tres pasándose*; ahora es **16,8 — exactamente un frame, cero perdidos, cero long tasks**. El presupuesto **no se movió** y no se mueve: 20/34/50 no son mediciones ratcheteadas sino física de 60 fps (§3.2), así que aflojarlos o apretarlos no es lo mismo que con P12 o P20. Lo que cambió es que ahora hay holgura donde había suerte.

- **52 nodos de tarjeta montados de 500 declarados**, sobre las cuatro columnas abiertas del `perf-500`. `ceil(900/128) + 2·overscan + 1 = 13` por columna, que es la cuenta de §2.1 con las alturas de N17 en vez de las 112 que tachó.
- **El documento SSR cae de 883.477 a 222.068 bytes** — 661 KB, un 75%. Medido con las dos versiones, no estimado.
- ⚠️ **Y EL TTI NO SE MOVIÓ: 2251 → 2248 ms.** Eso no es un fallo de la medición, es la respuesta: **FCP sale 2214 de esos 2248**, así que en esta ruta el TTI está clavado al primer pintado y el primer pintado **no lo domina el tamaño de este documento**. 661 KB menos valen 3 ms. Quien intente comprar TTI acá con más recorte de marcado va a gastar el día.
- **La otra mitad del contrato §2.1 NO está construida y por eso no cambió el payload:** *"el servidor devuelve 20 tarjetas por columna"* sigue sin existir, así que los **115.441 bytes de datos de hidratación de las 500 tarjetas viajan igual** — idénticos al byte antes y después. Esta entrega ahorra nodos y frames, no bytes ni trabajo de base.

🔴 **UN GUARDIÁN TUVO QUE CAMBIAR, y era el más caro de tocar.** `drag-perf.spec.ts` asserteaba `500 tarjetas montadas` **antes de medir**, y estaba escrito contra el fallo más caro que existe: un frame rate verde medido sobre una pantalla vacía. La virtualización lo hace falso a propósito, así que **no se borra: se parte en dos y hacen falta las dos.** El tablero tiene que **declarar** 500 (la suma de `data-cards`, que es el fixture llegando a la pantalla y no quedándose en la base) **y** el DOM tiene que tener muchas menos — que es la aserción de que la virtualización está encendida. Sin la segunda, una regresión que la apagara devolvería el tablero de 500 nodos y este gate volvería a medirlo en silencio.

**LA TERCERA REPRESENTACIÓN DE LA ALTURA, con su prueba de deriva.** El virtualizador multiplica por el pitch en JavaScript y §2.1 le **prohíbe leer layout** para averiguarlo (*"las lecturas de layout en el camino del drag son la causa más común de un frame perdido"*). Así que 120/156 ahora existen también en TypeScript, y `card-height.test.ts` compara **las tres por valor** — TS, CSS y `ref.ci_ratchet`. 🎯 **Probado por mutación:** bajar la constante de TS a 112 —el número que N17 tachó— pone el test rojo nombrando los dos valores.

- **Descartada a propósito la alternativa de leer `--card-h` del estilo computado**: no puede derivar, pero necesita parsear un float de un string CSS, y los parsers están prohibidos fuera de `app/lib/money/**`. Escribirlo como `Number.parseFloat` para que el selector del linter no lo vea sería **evadir una guarda, no satisfacerla**.

🔴 **LO QUE UNA VENTANA SE LLEVA PUESTO, dicho y después atado: una tarjeta fuera de la ventana no está en el DOM, así que no está en el orden de tabulación.** El tabulador igual llega, pero **por una cadena y no por una propiedad**: el foco se mueve a la última tarjeta montada → el navegador la desplaza a la vista → el scroll mueve la ventana → monta las siguientes → el Tab siguiente tiene dónde caer. Una cadena con un eslabón roto se ve idéntica a una que funciona hasta que la pisa alguien que no usa mouse.

- ✅ **Hay un test que la recorre entera** y 🎯 **quitar el listener de scroll pone TRES de los cuatro en rojo**, incluido ése.
- ⚠️ **Y una demora de 60 ms entre Tabs que es parte del diseño del test, no un parche:** sin ella pasaba sola y **fallaba dentro de la suite completa**. Playwright teclea a ~70 Tabs por segundo y una persona rápida hace ocho; en el medio entra un render de React. Medir la cadena más rápido de lo que ningún humano puede es medir el scheduler de React. Se assertea al doble de la velocidad de una persona.

**LA PÁGINA SIGUE SIENDO EL CONTENEDOR DE SCROLL**, y es una desviación deliberada del boceto C-32 (columna con su propio viewport y cabecera pegajosa). Scrollers por columna significan darle al tablero una altura acotada, y eso significa cambiar el layout del shell **para todas las pantallas** — en el único cambio cuyo objetivo entero es que el vendedor no vea ninguna diferencia. La cabecera pegajosa es otra historia y no es ésta.

- ⚠️ **CONSECUENCIA PARA VALIDAR EN PANTALLA, que es la regla de este proyecto: en el tenant del demo NO SE VE NADA.** El umbral es 30 tarjetas por columna y el demo tiene muchas menos, así que renderiza plano y todos los specs viejos siguen midiendo una columna entera y real. Para verlo en pantalla hay que entrar como el vendedor del fixture: **`perf500@perf.test` / `demo-password-1234`**, `/board`, y scrollear una columna de 125.
- **El reloj `NEW` queda DESBLOQUEADO y NO construido.** La entrada de abajo lo dejó anotado con su tabla: el problema eran 500 relojes, y ahora nunca hay más de ~13 por columna en el DOM. Es el próximo intento, no parte de éste.
...
> ✅ **RESUELTO el 2026-08-04** por la entrada de arriba: la virtualización era la respuesta real y era. Mediana 16,8 ms, 52 nodos en vez de 500. Este párrafo se conserva porque el diagnóstico fue correcto y la conducta —anotar el borde en vez de mover el presupuesto— es la que quedó bien.
...
| 12 | Drag a 60 fps con 500 tarjetas | ✅ **CERRADA.** Perfil `dnd-ci` (2× CPU throttle) sobre `perf-500`, **contra el BUILD DE PRODUCCIÓN**: **p95 16,8 ms · max 16,8 ms (cero frames perdidos) · cero long tasks**, contra p95≤20 / frame≤34 / longtask≤50. El fixture se assertea antes de medir, y el gate está probado con dientes (bloqueo de 60 ms → las tres aserciones rojas). **Desde el 2026-08-04 el max de la mediana es 16,8 y no 33,3**: la virtualización de la columna monta 52 nodos en vez de 500, y la aserción de fixture pasó de *"500 montadas"* a *"500 declaradas Y una ventana acotada"* — las dos hacen falta |
| 13 | Publicar las contradicciones | ✅ `docs/sprint-0/g13-published-contradictions.md` |

### ▶️ LO SIGUIENTE — sesión del 2026-08-04, cierre

**`master` = `origin/master` + commits locales sin empujar. 245 tests · 61 e2e por perfil funcional (122 casos entre los dos, más `dnd-ci` y `lh-ci`) · árbol limpio · demo sembrado.** La columna se virtualiza (entrada de arriba); el punto 1 de esta lista pasa de bloqueado a desbloqueado y sin construir.

⚠️ **Una corrección de contabilidad, no de trabajo:** no pude reproducir de dónde salía el *"83 e2e"* de las entradas anteriores — `npx playwright test --list` da **61 por perfil**, y una corrida completa de los dos perfiles reporta **88 pasando** (el resto se saltea por perfil). De acá en más se cuenta con el `--list`, que es un comando y no una memoria.

**Lo que NO depende de nadie — por acá seguir, en este orden:**

1. **El reloj `NEW` que tictaquea** — **DESBLOQUEADO el 2026-08-04, y no construido.** La condición que este punto ponía —*"no vuelve a intentarse sin virtualización del tablero"*— se cumplió: la columna se virtualiza y **nunca hay más de ~13 tarjetas por columna en el DOM**, así que los 500 relojes simultáneos que P6 rechazó dos veces ya no pueden existir. La tabla de la entrada del 2026-08-03 sigue siendo el punto de partida: tictaqueando todo dio 116,7 ms / 84 ms y acotado a lo visible 50,0 / 53,0, los dos contra 34 / 50 — **pero los dos se midieron sobre 500 nodos.** Hay que volver a medir, no suponer que ahora entra. Sigue haciendo falta **un** tick compartido y no trece.
...
- **La mitad SERVIDOR del contrato de virtualización (`04b` §2.1) no está construida:** *"el servidor devuelve 20 tarjetas por columna … el resto se busca al scrollear"*. Hoy el loader manda las 500 y **115 KB de datos de hidratación viajan aunque sólo se rendericen 64 tarjetas**. No es urgente —el TTI está clavado al FCP y no al tamaño del documento, medido el 2026-08-04— pero es la mitad que falta y el día que una columna tenga miles de tarjetas es la que va a doler. La propiedad que esa mitad protege (*"la suma de una columna nunca es incorrecta por culpa de la ventana"*) ya se cumple por otro motivo: el total y el conteo los calcula la base y el cliente no los toca.
- **Los ids de tenant y el umbral de virtualización se cruzan en un sitio incómodo:** el gate de a11y (axe) corre sobre el tenant del demo, que tiene menos de 30 tarjetas por columna, así que **el camino virtualizado no lo audita axe**. Lo cubren las aserciones de `column-virtualization.spec.ts`, que no es lo mismo.
